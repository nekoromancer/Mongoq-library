...
- w : 쓰기 연산(insert, remove, update)의 수행시 성공 여부를 기다립니다. 기본값은 1(true)입니다. 만일 0(false)로 설정될 경우 성공 실패 여부를 신경쓰지 않습니다.
- wTimeoutMS : w 옵션이 1(true)일 경우 성공 여부를 얼마동안 기다릴지 정의합니다.  단위는 밀리세컨드(ms)이며 기본값은 10000입니다(10초).  지정된 시간이 경과한 이후에도 결과가 반환되지 않을 경우 예외가 발생합니다.
...
## v. hint()
find() 메서드로 데이터를 인출할 때 어떤 인덱스로 검색할 것인지 힌트를 줍니다. 힌트는 ensuerIndex를 통해 인덱스로 지정된 Key 값이어야 합니다.  인덱스가 여러개 지정된 Collection에서 hint를 사용할 경우 반응 시간을 대폭 단축 할 수 있습니다.

```php
$hint = 'name'; // name필드는 인덱스로 지정되어 있어야 합니다.

$result = $this->mongoq->from('collection')->hint( $hint )->find();
```
